...
for (const codec of CODECS) {
    try {
        await send('browsingContext.navigate', {
            context, url: `http://127.0.0.1:${PORT}/live.html?codec=${codec}`, wait: 'complete'
        });
        ...
        const info = await evaluate('JSON.stringify(window.info)').then(JSON.parse);
        const glErr = await evaluate('window.glErr');
        const glRead = await evaluate('JSON.stringify(window.glRead || null)').then(JSON.parse);
        const c2Read = await evaluate('JSON.stringify(window.c2Read || null)').then(JSON.parse);

        const shot = await send('browsingContext.captureScreenshot', {
            context, clip: { type: 'box', x: 0, y: 0, width: W, height: H }
        });
        const png = decodePNG(Buffer.from(shot.data, 'base64'));
        const scale = png.width / W;
        if (!Number.isInteger(scale) || scale < 1 || scale > 3) {
            throw new Error(`unexpected screenshot scale ${scale} (${png.width}x${png.height})`);
        }
        const at = (x, y) => {
        ...
        const px = {};
        for (const [name, y] of Object.entries(ROWS)) px[name] = XS.map((x) => at(x, y));
        ...
        if (info.timedOut) console.log(`  TIMED OUT; stages: ${JSON.stringify(info.stages)}`);
        if (info.fatal) console.log(`  FATAL: ${info.fatal}`);
        if (glErr) console.log(`  WebGL error: ${glErr}`);
        console.log(`  screenshot scale: ${scale}x`);
        ...
        results.push({ engine: 'firefox', codec, negotiated: info.codec, max2D, maxGL, maxGLNC, maxBM, maxGLBM, maxGLCV, scale });
    } catch (e) {
        console.log(`\n=== firefox / ${codec} === FAILED: ${e.message.split('\n')[0]}`);
        results.push({ engine: 'firefox', codec, error: e.message.split('\n')[0] });
    }
}

function deserialize(v) {
    if (!v) return null;
    if (v.type === 'string' || v.type === 'number' || v.type === 'boolean') return v.value;
    if (v.type === 'null' || v.type === 'undefined') return null;
    return v.value ?? null;
}
